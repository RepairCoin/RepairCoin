...
import { getSupportTriage, SupportTriage } from '@/services/api/supportTriage';
          ...
          <div className="bg-[#1A1A1A] rounded-lg border border-gray-800 flex flex-col h-[600px]">
            {selectedTicket ? (
              <>
                ...
                {/* AI Triage (Support Ticket Triage) */}
                <SupportTriageCard ticketId={selectedTicket.id} />
...
const PRIORITY_STYLES: Record<string, string> = {
  ...
  low: "bg-gray-500/15 text-gray-400 border-gray-500/40",
};

function SupportTriageCard({ ticketId }: { ticketId: string }) {
  ...
  useEffect(() => {
    ...
    getSupportTriage(ticketId)
      .then((t) => { if (!cancelled) setTriage(t); })
  ...
  const copyReply = () => {
    ...
    navigator.clipboard.writeText(triage.suggestedReply);
    toast.success("Draft reply copied — paste into the message box");
  };

  return (
    <div className="mx-4 mt-3 rounded-lg border border-gray-700 bg-gray-900/60">
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        {triage && (
          <>
            <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${PRIORITY_STYLES[triage.suggestedPriority] || PRIORITY_STYLES.medium}`}>
            ...
            <span className="text-[11px] text-gray-400 uppercase tracking-wide px-2 py-0.5 rounded bg-gray-800">
              {triage.suggestedCategory.replace("_", " ")}
            </span>
          </>
        )}
      ...
      {open && (
        <div className="px-3 pb-3">
          {loading ? (
            ...
            <p className="text-xs text-gray-500">Triage unavailable.</p>
          ) : (
            <>
              <p className="text-sm text-gray-300">{triage.summary}</p>
              <div className="mt-2 rounded-md bg-[#111] border border-gray-800 p-2">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[11px] text-gray-500 uppercase tracking-wide">Suggested reply</span>
                  <button
                    onClick={copyReply}
                    className="flex items-center gap-1 text-[11px] text-[#FFCC00] hover:underline"
                  >
                    <Copy className="w-3 h-3" /> Copy
                  </button>
                </div>
                <p className="text-xs text-gray-300 whitespace-pre-wrap">{triage.suggestedReply}</p>
              </div>
              <p className="mt-2 text-[10px] text-gray-600">
                AI suggestion — review before sending or changing priority/category.
              </p>
            </>
          )}
        </div>
