const CheckIcon = () => (
  ...
  export const TierCard = ({ tierData }: { tierData: any }) => {
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-7xl mx-auto">
        {tierData.map((tier: any) => (
          ...
          >
            <div
              className="w-full flex flex-col md:gap-2 px-8 py-2 md:py-6 text-white rounded-t-3xl"
              style={{
            ...
            >
              <div className="flex flex-col gap-4">
                {tier.header && (
                  <p className="text-lg md:text-xl text-center  text-[#FFCC00] font-semibold mb-4">
                    {tier.header}
                  </p>
                )}
                {tier.subHeader && (
                  <p className="text-sm text-gray-100">{tier.subHeader}</p>
                )}
