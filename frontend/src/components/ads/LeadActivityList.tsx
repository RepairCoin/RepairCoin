...
// Inline lead-activity list (calls / emails / notes / status changes). Extracted from
// LeadActivityTimeline so it can live inside the Conversation modal's "Activity" tab as well as the
// standalone timeline dialog. Self-fetching + mode-aware.

import React, { useCallback, useEffect, useState } from "react";
import { Loader2, Phone, Mail, StickyNote, ArrowRightLeft } from "lucide-react";
...
export const LeadActivityList: React.FC<{ leadId: string; mode?: LeadMode; active?: boolean }> = ({ leadId, mode = "admin", active = true }) => {
  ...
  if (loading) {
    return <div className="flex items-center gap-2 text-gray-400 text-sm py-6 justify-center"><Loader2 className="w-4 h-4 animate-spin" /> Loading…</div>;
  }
  if (items.length === 0) {
    return <p className="text-sm text-gray-500 py-6 text-center">No activity yet. Calls, emails, notes and status changes show up here.</p>;
  }
  return (
