...
/**
 * Polls the customer's suspension status while the customer dashboard is active.
 * Redirects to the customer suspended screen when a suspension is detected.
 */
export function useCustomerSuspensionGuard() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (!result.exists || result.type !== "customer" || !result.user) return;
