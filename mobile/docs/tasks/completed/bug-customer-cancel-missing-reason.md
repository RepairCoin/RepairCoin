# Bug: Customer Cancel Booking Fails — Missing Cancellation Reason
...
| File | Change |
|------|--------|
| `mobile/shared/services/booking.services.ts` | Add `cancellationReason` and `cancellationNotes` params to `cancelOrder` |
| `mobile/feature/booking/hooks/mutations/useBookingMutations.ts` | Update mutation to accept reason and notes |
| `mobile/feature/booking/hooks/ui/useBookingDetail.ts` | Update `confirmCancel` to pass reason |
| `mobile/feature/booking/screens/BookingDetailScreen.tsx` | Add cancel reason modal or use Alert with options |
