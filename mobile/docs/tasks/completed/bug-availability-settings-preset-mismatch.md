# Bug: Availability Settings Preset Buttons Can't Display Non-Preset Values
...
| Setting | Web Input | Mobile Presets | Mismatch Risk |
|---------|-----------|---------------|---------------|
| Buffer Time | Free number input (any value) | [0, 5, 10, 15, 30] | High — e.g., 20 not in presets |
| Slot Duration | Free number input | [15, 30, 45, 60, 90, 120] | Medium — e.g., 20 not in presets |
| Advance Booking | Free number input | [7, 14, 30, 60, 90] | Medium — e.g., 10 not in presets |
| Min Booking Hours | Free number input | [1, 2, 4, 8, 12, 24] | Medium — e.g., 3 not in presets |
| Max Concurrent | Free number input | [1, 2, 3, 4, 5] | Low — most values covered |
...
| Approach | Pros | Cons |
|----------|------|------|
| Add all possible values to presets | Simple | Too many buttons, still can't cover every value |
| Replace with number input only | Full flexibility | Loses quick-select convenience |
...
| File | Change |
|------|--------|
| `mobile/feature/service/components/AvailabilityModal.tsx` | Update SettingCard to support hybrid preset + custom input |
