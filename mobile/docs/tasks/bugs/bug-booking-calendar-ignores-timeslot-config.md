# Bug: Booking Calendar Ignores Shop Time Slot Configuration

## Status: Open
## Priority: High
## Date: 2026-04-04
## Category: Bug - Appointment Booking
...
**Web behavior:** Shows the correct saved value (20 minutes on sc3.png, shown as 30 in a different state).
...
| Setting | DB Value (peanut) | Web | Mobile | Status |
|---|---|---|---|---|
| Slot Duration (60m) | 60 | Applied (backend) | Applied (backend) | OK |
| Buffer Time (20m) | 20 | Applied (backend) | Applied (backend) but display shows 0 | Bug 4 |
| Max Concurrent (1) | 1 | Enforced (backend) | Enforced (backend) | OK (verify) |
| Advance Booking (7 days) | 7 | Limits calendar to 7 days | Hardcoded to 30 days | **Bug 1** |
| Minimum Notice (2 hours) | 2 | Validates before payment | Not validated | **Bug 2** |
| Allow Weekend Booking | true | Checked | Not checked | Bug 5 |
...
| File | Change |
|------|--------|
| `mobile/feature/appointment/screens/AppointmentCompleteScreen.tsx` | Fetch time slot config via public endpoint, pass to schedule screen |
| `mobile/feature/appointment/screens/AppointmentScheduleScreen.tsx` | Use `bookingAdvanceDays` for maxDate, check `allowWeekendBooking` |
| `mobile/feature/appointment/types.ts` | Add time slot config to screen props |
| `mobile/feature/service/components/AvailabilityModal.tsx` | Add 20 to buffer time preset options |
...
5. **After fix:** Only dates within 7 days should be selectable (April 4-11)
