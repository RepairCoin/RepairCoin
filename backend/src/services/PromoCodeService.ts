...
export class PromoCodeService {
  ...
  // --- Admin management (platform-wide, not shop-scoped) ---

  async adminSetPromoActive(promoCodeId: number, isActive: boolean): Promise<PromoCode> {
    const existing = await this.promoCodeRepo.findById(promoCodeId);
    if (!existing) {
      throw new Error('Promo code not found');
    }
    const updated = await this.promoCodeRepo.update(promoCodeId, { is_active: isActive });
    if (!updated) {
      throw new Error('Failed to update promo code');
    }

    eventBus.publish({
      type: isActive ? 'promo_code:activated' : 'promo_code:deactivated',
      aggregateId: existing.shop_id,
      timestamp: new Date(),
      source: 'PromoCodeService',
      version: 1,
      data: { shopId: existing.shop_id, promoCodeId, code: existing.code, by: 'admin' }
    });
  ...
  async adminDeletePromoCode(promoCodeId: number): Promise<void> {
    ...
    const ok = await this.promoCodeRepo.delete(promoCodeId);
    if (!ok) {
      throw new Error('Failed to delete promo code');
    }

    eventBus.publish({
      type: 'promo_code:deleted',
      aggregateId: existing.shop_id,
      timestamp: new Date(),
      source: 'PromoCodeService',
      version: 1,
