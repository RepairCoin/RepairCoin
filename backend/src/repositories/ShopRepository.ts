...
export class ShopRepository extends BaseRepository {
  ...
  async getShopPurchase(purchaseId: string): Promise<any | null> {
    try {
      const query = 'SELECT * FROM shop_rcn_purchases WHERE id = $1';
      const result = await this.pool.query(query, [purchaseId]);
      ...
      logger.error('Error fetching shop purchase:', error);
      throw new Error('Failed to fetch shop purchase');
    }
  }
