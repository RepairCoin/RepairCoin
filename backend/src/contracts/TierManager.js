...
class TierManager {
    ...
    calculateTier(lifetimeEarnings) {
        if (lifetimeEarnings >= this.TIER_THRESHOLDS.GOLD.min)
            return "GOLD";
        if (lifetimeEarnings >= this.TIER_THRESHOLDS.SILVER.min)
            return "SILVER";
        return "BRONZE";
    }
    ...
    canEarnThisMonth(customer, tokensToEarn) {
    ...
    canRedeemAtShop(customer, redemptionAmount, isHomeShop) {
