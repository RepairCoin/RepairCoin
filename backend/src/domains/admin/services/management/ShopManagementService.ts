...
export class ShopManagementService {
  async getShops(filters: ShopFilters) {
    try {
      ...
      const { tokenOperationsService } = await import(
        "../operations/TokenOperationsService"
      );
      const shopsWithPendingMints = await Promise.all(
        result.items.map(async (shop) => {
          try {
            const pendingMintAmount =
              await tokenOperationsService.getShopPendingMintAmount(
                shop.shopId
              );
            return {
              ...shop,
            ...
            logger.error(
              `Error getting pending mint amount for shop ${shop.shopId}:`,
              error
            );
            return {
              ...shop,
  ...
  async createShop(shopData: {
    ...
    try {
      ...
      const existingShops = await shopRepository.getShopsPaginated({
        page: 1,
        limit: 1000,
      });
      const shopWithWallet = existingShops.items.find(
        (s) =>
  ...
  async approveUnsuspendRequest(
    ...
    try {
      ...
      if (request.entityType === "customer") {
        const { customerManagementService } = await import(
          "./CustomerManagementService"
        );
        await customerManagementService.unsuspendCustomer(
          request.entityId,
          adminAddress
        );
      } else if (request.entityType === "shop") {
