...
router.get("/:address", async (req, res) => {
  try {
    ...
    const balanceInfo = await customerBalanceService.getCustomerBalanceInfo(
      address
    );
...
router.post("/:address/queue-mint", async (req, res) => {
  try {
    ...
    const validation = await customerBalanceService.validateMintRequest(
      address,
      amount
    );
    if (!validation.valid) {
    ...
    // Queue for minting
    const mintRequest = await customerBalanceService.queueForMinting(
      address,
      amount
    );
...
router.post("/:address/sync", async (req, res) => {
  try {
    ...
    const balanceInfo = await customerBalanceService.syncCustomerBalance(
      address
    );
