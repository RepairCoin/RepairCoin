  ...
  describe("2. Referral Edge Cases", () => {
    describe("2.1 Referral Code Edge Cases", () => {
      it("should handle case-insensitive referral codes", async () => {
        ...
        // Resolve the referrer only for the submitted referral code — the random
        // code generated by generateUniqueReferralCode() must return no match so
        // registration can issue a unique code.
        mockCustomerRepo.getCustomer.mockResolvedValue(null);
        mockCustomerRepo.getCustomerByReferralCode.mockImplementation(
          async (code: string) =>
            code && code.toUpperCase() === "ABCD1234" ? (mockReferrer as any) : null
        );
      ...
      it("should reject self-referral", async () => {
        ...
        mockCustomerRepo.getCustomer.mockResolvedValue(null);
        // Resolve the referrer only for the submitted code; the generated unique
        // code lookup must return no match.
        mockCustomerRepo.getCustomerByReferralCode.mockImplementation(
          async (code: string) =>
            code && code.toUpperCase() === "SELF1234" ? (selfCustomer as any) : null
        );
        mockCustomerRepo.createCustomer.mockResolvedValue({} as any);
