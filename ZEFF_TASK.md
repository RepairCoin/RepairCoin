...
5. **Frontend/mobile tracks (later, breaking):** the remaining frontend track is removing wallet-only **login** (the action-feature hiding is done in #3; login removal is the breaking change that needs a deprecation notice). **Mobile is fully untouched** — wallet connect screen, redemption signature, thirdweb constants still assume blockchain.
