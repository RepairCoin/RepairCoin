...
**Four follow-up fixes folded into this branch alongside fix-7:**
...
3. **WebSocket auto-reconnect + heartbeat** (`frontend/src/hooks/useNotifications.ts`) — discovered when bidirectional human-to-human messaging stopped working ~43 minutes after the last activity in a chat. Three pre-existing bugs in the WS client: (a) `ws.onclose` with code 1006 short-circuited reconnect entirely — but 1006 fires for backend-deploy restarts, network blips, idle timeouts, and tab throttling, all of which should retry; (b) no client-side heartbeat (backend handles `ping`/`pong` but the frontend never sent any), so the connection went "zombie" — TCP open but server already dropped it, no close event, customer keeps a dead socket; (c) `maxReconnectAttempts=5` with 30s capped backoff = ~1 minute of retry, too short for typical 60-90s deploy windows. Fix: send `{type:"ping"}` every 25s with a 10s pong watchdog (force-close on miss → reconnect), removed the 1006-bails-out branch, bumped attempts to 20 (~10 min retry), added visibility-change reconnect for backgrounded tabs whose sockets died.

4. **AI reply WS broadcast missing shop subscriber** (`backend/src/domains/messaging/services/MessageService.ts` + `OrderConfirmationHandler.ts`) — found while diagnosing the bidirectional sync issue above. Even with a healthy WS, the shop's chat tab never received `message:new` broadcasts for AI replies, because both AI-reply broadcast paths (`fireAiAutoReply` and `OrderConfirmationHandler`) targeted ONLY the customer's wallet address. Result: shop owners couldn't see what the AI was saying on their behalf in real-time and had to refresh the chat to monitor AI conversations. Pre-existing since Task 8, just not visible until we tested the shop side. Fix: include `shop.walletAddress` in the broadcast target list alongside the customer's. For `fireAiAutoReply` the shop is looked up via `shopRepository.getShop(shopId)`; `OrderConfirmationHandler` already had `shop` loaded earlier in the function so reused that. Defensive: shop lookup failure logs but doesn't suppress the customer broadcast (degrade to single-target rather than silent). 2 new tests added to `OrderConfirmationHandler.test.ts` asserting the shop is included when `walletAddress` is set, and falling back gracefully when it isn't. Test count: 236 → 238.
