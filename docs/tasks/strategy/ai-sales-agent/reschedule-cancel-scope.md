...
| # | Decision | Locked |
|---|---|---|
| Q1 | **Cancel reason — optional.** Add optional `reason?: string` to the cancel endpoint; persist to `service_orders.cancellation_notes` (the existing free-form column from migration 051 — see impl doc §5 schema correction). `cancellation_reason` (the existing code column) gets `'customer_cancelled'` for chat-initiated cancels. Surfaces to shop dashboard as "Cancelled — '<reason>'" and enables a future Insights tool to answer "why are people cancelling?" | ✅ |
